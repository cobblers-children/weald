defmodule Weald.PomodoriTest do
  ...
  describe "pomodoro" do
    ...
    test "create_pomodoro/1 with valid data creates a pomodoro" do
      valid_attrs = %{remaining: 42, due_at: ~U[2025-01-28 06:35:00Z], finished_at: ~U[2025-01-28 06:35:00Z]}

      assert {:ok, %Pomodoro{} = pomodoro} = Pomodori.create_pomodoro(valid_attrs)
      assert pomodoro.remaining == 42
      assert pomodoro.due_at == ~U[2025-01-28 06:35:00Z]
      assert pomodoro.finished_at == ~U[2025-01-28 06:35:00Z]
    end
    ...
    test "update_pomodoro/2 with valid data updates the pomodoro" do
      pomodoro = pomodoro_fixture()
      update_attrs = %{remaining: 43, due_at: ~U[2025-01-29 06:35:00Z], finished_at: ~U[2025-01-29 06:35:00Z]}

      assert {:ok, %Pomodoro{} = pomodoro} = Pomodori.update_pomodoro(pomodoro, update_attrs)
      assert pomodoro.remaining == 43
      assert pomodoro.due_at == ~U[2025-01-29 06:35:00Z]
      assert pomodoro.finished_at == ~U[2025-01-29 06:35:00Z]
    end
  ...
  describe "pomodoro" do
    ...
    @invalid_attrs %{running: nil, stage: nil, remaining: nil, due_at: nil, finished_at: nil}
    ...
    test "create_pomodoro/1 with valid data creates a pomodoro" do
      valid_attrs = %{running: true, stage: :pomodoro, remaining: 42, due_at: ~U[2025-01-28 23:03:00Z], finished_at: ~U[2025-01-28 23:03:00Z]}

      assert {:ok, %Pomodoro{} = pomodoro} = Pomodori.create_pomodoro(valid_attrs)
      assert pomodoro.running == true
      assert pomodoro.stage == :pomodoro
      assert pomodoro.remaining == 42
      assert pomodoro.due_at == ~U[2025-01-28 23:03:00Z]
      assert pomodoro.finished_at == ~U[2025-01-28 23:03:00Z]
    end
    ...
    test "update_pomodoro/2 with valid data updates the pomodoro" do
      pomodoro = pomodoro_fixture()
      update_attrs = %{running: false, stage: :break, remaining: 43, due_at: ~U[2025-01-29 23:03:00Z], finished_at: ~U[2025-01-29 23:03:00Z]}

      assert {:ok, %Pomodoro{} = pomodoro} = Pomodori.update_pomodoro(pomodoro, update_attrs)
      assert pomodoro.running == false
      assert pomodoro.stage == :break
      assert pomodoro.remaining == 43
      assert pomodoro.due_at == ~U[2025-01-29 23:03:00Z]
      assert pomodoro.finished_at == ~U[2025-01-29 23:03:00Z]
    end
